...
Partial Class ManageSys_ApplyUser_PlanBook
    ...
    Sub BindData()
        ...
        Dim nCondition As String = "PrId=" & gPrId & "and PrFiClass='项目计划书'"
        gvPlanBook.DataSource = oProjectFile.GetToDV(nCondition)
        gvPlanBook.DataBind()
    End Sub

    Protected Sub btnOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOk.Click
        Dim nSavePath As String = ""
        If fupPlanBook.HasFile Then
            Dim nPrFiName As String = Server.HtmlEncode(fupPlanBook.FileName)
            ...
            If nExtension = ".doc" Or nExtension = ".docx" Then
                ...
                fupPlanBook.SaveAs(nSavePath)
                Dim oProjectFile As New ProjectFile
                oProjectFile.PrId = gPrId
                oProjectFile.PrFiName = nPrFiName
                oProjectFile.PrFiAddress = nPrFiAddr
                oProjectFile.PrFiClass = "项目计划书"
                Dim nRePrFiId As Integer = 0
                nRePrFiId = oProjectFile.Add(oProjectFile)
                If nRePrFiId <> 0 Then
                    LogsOper.ShowInfotoWeb("文件上传成功！", upPlanBookPage)
                    '绑定数据到gvApplyBook中去
                    BindData()
                End If
            Else
                LogsOper.ShowInfotoWeb("只能上传.doc格式或.docx格式的文件！", upPlanBookPage)
                Exit Sub
            End If
        Else
            LogsOper.ShowInfotoWeb("请选择要上传的文件！", upPlanBookPage)
            Exit Sub
        End If
    End Sub

    Protected Sub gvPlanBook_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvPlanBook.RowCommand
        Try
            Dim nPrFiId As Integer = gvPlanBook.DataKeys(e.CommandArgument).Value.ToString
            If e.CommandName = "deleteFile" Then
                ...
                If nReDelete <> 0 Then
                    LogsOper.ShowInfotoWeb("删除成功！", upPlanBookPage)
                    '绑定数据到gvApplyBook中去
                    BindData()
                End If
            End If
